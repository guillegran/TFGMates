...
\RR^n	\arrow{r}{\varphi^F}\arrow{d}{ } & M_a\arrow{ld}[anchor=north,rotate=30]{\theta} \\ 
	 \TT^n.&
       \end{tikzcd}
     \end{center}
     Igual que en la demostración del teorema de Arnold-Liouville, la construcción del difeomorfismo $\theta$ depende de la elección del punto $x\in M_a$. Para escoger puntos en los toros cercanos $M_{b}\subset U$, consideremos un entorno abierto $\mathscr{D} \subset U$ de $x$ difeomorfo a $D\times \RR^n$, con $D\subset \RR^n$ un disco abierto en torno a $a$, de modo que $F(\mathscr{D})=D$. Tomamos entonces un punto $x_b \in M_{b}$ tal que $x_b\in \mathscr{D}$ y construimos análogamente un difeomorfismo $\theta_b:M_b \rightarrow \TT^n$. Nótese además que nos hemos cuidado de que la construcción de este $\theta_b$ dependa diferenciablemente del parámetro $b$.
     Con todo esto, tenemos que la aplicación
     \begin{align*}
